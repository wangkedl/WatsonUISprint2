...
class JFShoppingCartViewController: UIViewController {
    ...
    private func prepareUI() {
        ...
        // 导航栏左边返回
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Back", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(JFShoppingCartViewController.didTappedBackButton))
    ...
    lazy var selectButton: UIButton = {
        ...
        selectButton.setTitle("Select All", forState: UIControlState.Normal)
        ...
        selectButton.addTarget(self, action: #selector(JFShoppingCartViewController.didTappedSelectButton(_:)), forControlEvents: UIControlEvents.TouchUpInside)
...
extension JFShoppingCartViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        detailView.showAddGoodButton = false
...
extension JFShoppingCartViewController {
    ...
    private func reCalculateGoodCount() {
        ...
        for model in addGoodArray! {
            ...
            // 只计算选中的商品
            if model.selected == true {
                price += Float(model.count) * Float(model.newPrice)!
            }
        }
        
        // 赋值价格
        let attributeText = NSMutableAttributedString(string: "Total：\(self.price)0")
        attributeText.setAttributes([NSForegroundColorAttributeName : UIColor.redColor()], range: NSMakeRange(6, attributeText.length - 6))
        totalPriceLabel.attributedText = attributeText
